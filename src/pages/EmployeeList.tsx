...
interface Employee {
  ...
  designation: string; status: EmployeeStatus; joinDate: string; type: string;
}

const statusClass: Record<EmployeeStatus, string> = {
  Active: "status-active", Onboarding: "status-pending", "On Leave": "status-pending",
  Resigned: "status-resigned", Inactive: "status-inactive", "Notice Period": "status-notice",
};
...
const departments = ["Engineering", "Marketing", "HR", "Operations", "Finance", "Design", "Support"];
...
export default function EmployeeList() {
  ...
  const [newEmp, setNewEmp] = useState({ name: "", email: "", phone: "", dob: "", department: "", designation: "", type: "Full-time", joinDate: "" });

  const filtered = employees.filter(e => {
    const matchSearch = e.name.toLowerCase().includes(search.toLowerCase()) || e.id.toLowerCase().includes(search.toLowerCase()) || e.department.toLowerCase().includes(search.toLowerCase());
  ...
  const handleAddEmployee = () => {
    if (!newEmp.name || !newEmp.email || !newEmp.department) return;
    const id = `EMP-${String(employees.length + 1001).padStart(4, '0')}`;
    setEmployees(prev => [...prev, { id, name: newEmp.name, email: newEmp.email, department: newEmp.department, designation: newEmp.designation, status: "Onboarding", joinDate: newEmp.joinDate || new Date().toISOString().split('T')[0], type: newEmp.type }]);
    setNewEmp({ name: "", email: "", phone: "", dob: "", department: "", designation: "", type: "Full-time", joinDate: "" });
    setAddDialog(false);
  ...
  const handleDeactivate = (id: string) => {
    setEmployees(prev => prev.map(e => e.id === id ? { ...e, status: "Inactive" as EmployeeStatus } : e));
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-4">
      <motion.div variants={item} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground"><span className="font-mono-data">{employees.length}</span> total employees</p>
        </div>
        {isHR && (
          <Dialog open={addDialog} onOpenChange={setAddDialog}>
            <DialogTrigger asChild>
              <Button size="sm" className="gap-1.5 press-effect"><Plus className="w-3.5 h-3.5" />Add Employee</Button>
            </DialogTrigger>
            <DialogContent className="max-w-lg">
              <DialogHeader><DialogTitle>Add New Employee</DialogTitle></DialogHeader>
              <div className="space-y-4 pt-2">
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-xs text-muted-foreground mb-1 block">Full Name *</label><Input value={newEmp.name} onChange={e => setNewEmp({ ...newEmp, name: e.target.value })} placeholder="Enter full name" className="h-9 text-sm" /></div>
                  <div><label className="text-xs text-muted-foreground mb-1 block">Email *</label><Input value={newEmp.email} onChange={e => setNewEmp({ ...newEmp, email: e.target.value })} placeholder="email@company.com" type="email" className="h-9 text-sm" /></div>
                  <div><label className="text-xs text-muted-foreground mb-1 block">Phone</label><Input value={newEmp.phone} onChange={e => setNewEmp({ ...newEmp, phone: e.target.value })} placeholder="+977-98XXXXXXXX" className="h-9 text-sm" /></div>
                  <div><label className="text-xs text-muted-foreground mb-1 block">Date of Birth</label><Input type="date" value={newEmp.dob} onChange={e => setNewEmp({ ...newEmp, dob: e.target.value })} className="h-9 text-sm" /></div>
                  ...
                  <div><label className="text-xs text-muted-foreground mb-1 block">Designation *</label><Input value={newEmp.designation} onChange={e => setNewEmp({ ...newEmp, designation: e.target.value })} placeholder="e.g., Sr. Developer" className="h-9 text-sm" /></div>
                  ...
                  <div><label className="text-xs text-muted-foreground mb-1 block">Date of Joining</label><Input type="date" value={newEmp.joinDate} onChange={e => setNewEmp({ ...newEmp, joinDate: e.target.value })} className="h-9 text-sm" /></div>
                </div>
      ...
      <motion.div variants={item} className="space-y-3">
        <div className="flex items-center gap-2">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input placeholder="Search by name, ID, department…" value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-8 text-sm bg-card border-border" />
          </div>
        ...
        {showFilters && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex items-center gap-3 pb-1">
            <div className="flex items-center gap-2">
      ...
      <motion.div variants={item} className="bg-card border border-border rounded-lg overflow-hidden">
        <table className="nexus-table">
          <thead><tr><th>ID</th><th>Employee</th><th>Department</th><th>Designation</th><th>Type</th><th>Status</th><th>Joined</th><th className="w-10"></th></tr></thead>
          <tbody>
            {filtered.map(emp => (
              <tr key={emp.id} className="cursor-pointer" onClick={() => navigate(`/employees/${emp.id}`)}>
                ...
                <td>
                  <div className="flex items-center gap-2.5">
                    ...
                    <div><p className="text-sm font-medium leading-none">{emp.name}</p><p className="text-xs text-muted-foreground mt-0.5">{emp.email}</p></div>
                  </div>
                </td>
                ...
                <td onClick={e => e.stopPropagation()}>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild><button className="p-1 rounded hover:bg-muted transition-colors"><MoreHorizontal className="w-4 h-4 text-muted-foreground" /></button></DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-44">
                      ...
                      <DropdownMenuItem className="text-destructive" onClick={() => handleDeactivate(emp.id)}>Deactivate</DropdownMenuItem>
                      {isHR && <DropdownMenuItem className="text-destructive" onClick={() => handleDeleteEmployee(emp.id)}>Delete Employee</DropdownMenuItem>}
                    </DropdownMenuContent>
                  </DropdownMenu>
                </td>
              </tr>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="text-center text-sm text-muted-foreground py-8">No employees found matching your filters.</td></tr>
            )}
          </tbody>
        </table>
      </motion.div>
